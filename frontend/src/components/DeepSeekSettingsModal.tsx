...
export default function DeepSeekSettingsModal({ onClose }: Props) {
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="field-group">
            ...
            <input type="text" value={apiBaseUrl} onChange={(e) => setApiBaseUrl(e.target.value)} className="text-input" />
          </div>

          <div className="field-group">
            <label className="field-label">模型名称</label>
            <input type="text" value={model} onChange={(e) => setModel(e.target.value)} className="text-input" />
          </div>
          ...
          <div className="test-row">
            <button className="btn-secondary" onClick={handleTest} disabled={testStatus === 'testing' || !apiKey}>
              {testStatus === 'testing'
                ...
                : <><Wifi size={13} /><span>连接测试</span></>
              }
            </button>
            {testStatus === 'ok' && (
              <span className="test-ok"><CheckCircle2 size={13} /> 连接成功</span>
            )}
            {testStatus === 'fail' && (
              <span className="test-fail"><XCircle size={13} /> {testError}</span>
            )}
          </div>
