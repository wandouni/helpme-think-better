...
import { rawTextToMindmap } from '../utils/rawTextMindmap'
...
export default function LeftPanel() {
  ...
  const mindmapContent = useMemo(
    () => isRawFile ? rawTextToMindmap(markdown, activeFile?.filename ?? '原文') : markdown,
    [activeFile?.filename, isRawFile, markdown],
  )
    ...
    <div className="panel left-panel">
      <div className="panel-tabs">
        <button
          ...
          title={isRawFile ? '按原文内容展示思维导图' : '思维导图'}
        >
          <Share2 size={11} />
          思维导图
        </button>
